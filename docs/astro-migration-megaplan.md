...
- [x] Generator V2 has been simplified toward a lighter operator product instead of a heavier governance system: the Studio pane now shows clearer selected-combination context, preview input details are more editorial-friendly, deterministic meta descriptions are trimmed to safer lengths, CTA labels adapt more naturally to offer/preset context, and the reusable visual library now includes broader multi-jasa styles such as `immersive-story` and `trust-matrix`. Verification note: `studio` typecheck, `frontend` typecheck, focused generator QA tests, visual-library seed syntax check, live reseed to the `development` dataset, live generator smoke verification, and `git diff --check` all passed.
- [x] Generator V2 visual coverage is now richer at the block level, not only the preset level: the visual library expanded again to include `authority-canvas`, `offer-funnel`, and `process-mosaic`, while generator section variants can now emit existing repo blocks such as `stats-hero-block`, `benefits-block`, `features-package-block`, and `company-info`. Verification note: `studio` typecheck, `frontend` typecheck, focused generator QA tests, live reseed to the `development` dataset, live smoke verification, and `git diff --check` all passed after the block-level expansion.
- [x] Generator V2 operator workflow is now materially more usable inside Studio: the `Generator Run` pane no longer locks preview/generation to the first dataset items, and now lets the operator choose a keyword set plus row, run a selected dry run, and generate only the selected draft before using broader batch actions. Verification note: `studio` typecheck and `git diff --check` both passed after the pane upgrade.
- [x] Generator V2 now has a finished dev-only sample path, not just starter scaffolding: a new script can seed a complete `generatorTemplate`, `generatorDataset`, and `generatorProgram` under `/sample-generator`, then generate three matched draft sample pages (`website/jakarta`, `software/surabaya`, `printing/bandung`) into the development dataset with deterministic draft ids. Verification note: dry run, live dev write, matched-sample refresh, `frontend` typecheck, and `git diff --check` all passed.
- [x] Generator V2 now has an explicit Studio QA gate before writes: selected preview drafts are assessed for duplicate lineage/slug conflicts, SEO title and description ranges, slug safety, required block presence, and token coverage; blocked drafts cannot be written from the selected action, and batch dry runs now report QA-blocked combinations instead of treating them as writable output. Verification note: `studio` typecheck, focused generator QA tests, and `git diff --check` all passed.
- [x] Active runtime SEO/domain outputs are now cleaned of the last hardcoded Kotacom URL assumptions: `/llms.txt` and `/llms-full.txt` are generated dynamically from live Sanity data instead of static files, blog share URLs now follow the Sanity-managed site URL, the rewrite WhatsApp hero CTA no longer depends on `NEXT_PUBLIC_SITE_URL`, and shared metadata/JSON-LD helpers now resolve site URLs Sanity-first with relative fallback instead of env-owned absolute domains. Verification note: `frontend` typecheck, `studio` typecheck, and `git diff --check` all passed after the runtime cleanup.
- [x] Global SEO behavior is now substantially Sanity-first across the live frontend: `seoSettings` now controls canonical site URL, WebSite search path, robots AI crawler allowlist, sitemap static routes, and template-route sitemap inclusion; root `Organization` / `WebSite` / `LocalBusiness` JSON-LD is now built from Sanity `settings` + `seoSettings`; homepage fallback metadata now reuses Sanity defaults; and active page-level JSON-LD builders now receive the Sanity-managed site URL for absolute schema links. Verification note: `frontend` typecheck, `studio` typecheck, `studio` typegen, and `git diff --check` all passed after the cross-layer sync.
- [x] Sanity block insertion defaults have been hardened across the Studio block library: every schema under `studio/schemas/blocks` now has a top-level `initialValue`, and blocks with required fields or custom validation now start from valid payloads rather than empty objects. Verification note: a re-audit confirmed full top-level `initialValue` coverage in the block directory, and `studio` typecheck passed after the schema updates.
- [x] Generator and Service Type Studio lists now satisfy Sanity orderable-desk requirements: `generatorTemplate`, `generatorProgram`, `generatorDataset`, and `serviceType` all expose `orderRank` through `orderRankField(...)`, removing the schema mismatch that caused orderable list runtime errors in Studio. Verification note: `studio` typecheck passed, and `studio/schema.json` was refreshed after the schema fix.
- [x] Generator V2 in the development Studio is now a reusable visual template library instead of service-specific starter clutter: the Sanity-side `generatorTemplate` model now carries `visualPreset`, `motionPreset`, and style notes; section variants use supported block-type options plus optional `colorVariant` overrides; the renderer can now emit richer reusable blocks such as `split-row`, `timeline-row`, `cta-1`, `stats-hero-block`, `benefits-block`, `features-package-block`, and `company-info`; and the development dataset has been reseeded from old `website/software/printing` starters into a shared multi-jasa visual library (`editorial-grid`, `proof-showcase`, `pricing-spotlight`, `conversion-stack`, `immersive-story`, `trust-matrix`, `authority-canvas`, `offer-funnel`, `process-mosaic`) with one reusable dataset and nine programs under `/layanan`. Verification note: `studio` typecheck, `frontend` typecheck, generator render tests, Studio schema typegen, dev reseed write, and live generator smoke verification all passed against the development dataset.
- [x] Generator starter families now use the old page-generator output mainly as a visual template, not as a runtime system dependency: the seeded `generatorTemplate` families were restructured around visual-first sections (`hero`, `highlights`, `serviceTypes`, `pricing`, `testimonials`, `faq`, `finalCta`), and the deterministic renderer now supports `service-types-block` and `testimonials-block` in addition to the earlier hero/value/pricing/FAQ blocks. Verification note: `studio` typecheck, `frontend` typecheck, generator render tests, and script syntax checks all passed before reseeding the updated starter families to the development dataset.
- [x] Generator V2 now has a practical dataset input path and more usable deterministic output: `generatorDataset` can operate in `csv-ready` mode with pasted `keywordSetCsv` / `rowCsv` inputs, a new sync script can convert those pasted CSV blobs into normalized `keywordSets` and `rows`, and generated pages now include SEO metadata plus `pricing-block` support so starter families render more complete previews. Verification note: `studio` typecheck, `frontend` typecheck, generator render tests, and Node syntax checks all passed locally; rerunning the live dataset sync/reseed scripts was blocked in this cycle by the current approval-layer usage limit, so this batch was verified locally rather than by a second live Sanity write.
- [x] The development Sanity generator workspace is now cleaned of the old one-off printing seed and replaced with a consistent starter set: legacy `generator-template-printing-dev` / `generator-dataset-printing-dev` / `generator-program-printing-dev` were removed from the `development` dataset, and only three aligned starter families now remain (`website`, `software`, `printing`) as `generatorTemplate` + `generatorDataset` + `generatorProgram` triplets. Verification note: a live post-cleanup query against the development dataset now returns exactly 9 generator docs, with no leftover legacy seed IDs.
- [x] Legacy template migration tooling is now ready for the Generator V2 path: the remaining misleading `variant` field is hidden from `pageTemplate` editors, and a new development-only migration script can read legacy `pageTemplate` docs and upsert minimal `generatorTemplate` documents into the development dataset. Verification note: `studio` typecheck and `frontend` typecheck both pass, and a live dry run against the current development dataset succeeded with `writeMode: false`; the result was a no-op because the development dataset currently contains `0` legacy `pageTemplate` documents to migrate.
- [x] Legacy templating schemas in Studio are now explicitly minimized for editors without deleting stored data: `pageTemplate`, `pageLocation`, and `serviceLocation` are relabeled as legacy documents; inert legacy-only fields such as template/location blocks and top-block split settings are hidden from Studio; and `contentStatus` is kept visible only as an internal indexing-tool signal with a clarified description. Verification note: `studio` and `frontend` typecheck both still pass, so the cleanup changes operator UX without altering active frontend routing/render behavior.
- [x] Legacy runtime templating has been simplified without changing the live allowed-route contract: unreachable template branches were removed from `/about`, `/about/[slug]`, `/privacy`, and `/sistem-pos`; dead resolver exports for legacy template blocks/top-block counts were removed; and the template-page GROQ payload no longer fetches unused `slug`, `contentStatus`, `blocks`, `topBlockCount`, `isHybrid`, or `topBlockCountDefault` fields. Verification note: `frontend` typecheck and `studio` typecheck both still pass, so the active template routes remain intact while dead branches and payload bloat are reduced.
- [x] Studio content operations are now visually split between the new dev-only generator workflow and the old runtime templating system: `Generator` remains the primary dev-only workspace for `generatorProgram` / `generatorTemplate` / `generatorDataset`, while legacy `pageTemplate`, `pageLocation`, and `serviceLocation` documents are now grouped under a dedicated `Legacy Templating` desk section. This keeps the migration boundary explicit for editors without changing any frontend runtime contracts. Verification note: `studio` typecheck and the generator structure smoke check both still pass after the desk reorganization; a leftover untracked `studio/tmp/` artifact is now ignored in `studio/.gitignore`, but deleting the already-created local temp folder itself is still pending because the destructive shell command was blocked by the tool approval layer.
- [x] Task 7 of `Sanity Generator V2` is now implemented in the frontend verification layer: the shared `page` GROQ query now exposes generator lineage metadata for QA/debug reads, a focused frontend fetch helper can inspect that metadata with published or draft perspective, and the generator smoke script now self-bootstraps under `node` with TS support while preferring token-backed development `drafts` reads before falling back to fixtures. Verification note: `studio` typecheck, `frontend` typecheck, the dev write guard, the legacy export, and the live-dev smoke run all passed; the smoke output now resolves `source: "sanity-development"` instead of the older fallback path when development credentials are available.
- [x] Task 6 of `Sanity Generator V2` is now implemented as a read-only legacy inventory pass: a new mapper in `studio/lib/generator/legacy.ts` normalizes legacy `pageTemplate` records into minimal generator seed metadata, and a read-only export script now snapshots `pageTemplate`, `pageLocation`, and `serviceLocation` into `frontend/tmp/generator-legacy-template-inventory.json` for migration planning without writing any public content. Verification note: the export completed against the current Sanity dataset and reported template/location counts plus missing-template-reference counts; the mapper is intentionally narrow and currently preserves only template family, shell, top-block default, and source lineage fields rather than attempting full section migration.
- [x] Task 5 of `Sanity Generator V2` is now implemented in Studio: the generator has an explicit dev-only write helper plus deterministic generated draft IDs, a matching env guard script, and a development-only example seed script for `generatorTemplate` / `generatorDataset` / `generatorProgram`; the `generatorProgram` run pane now validates the current dataset, performs a pre-write dry run, and creates only missing `drafts.generator-page-<slug>` `page` documents while skipping/conflicting existing slug or lineage matches. Verification note: env guard, `studio` typecheck, `frontend` typecheck, and the live development seed script all passed; the earlier smoke fallback concern is now closed by Task 7's token-backed live-dev read path.
- [x] Task 4 of `Sanity Generator V2` is now implemented in Studio: the `generatorProgram` run pane resolves the selected `generatorTemplate` and `generatorDataset`, chooses the first dataset `keywordSet` plus first `row`, and builds a deterministic preview draft through `buildGeneratedPageDraft`; the same pane now supports a dry-run-only batch summary across all keyword-set x row combinations with duplicate checks against existing page slug and generator lineage, and a dev-only smoke script now verifies the dry-run path from the frontend workspace. Historical note: the first smoke pass used the explicit sample fallback before development seed data and the later Task 7 live-read fix were in place.
...
- [x] Remove local runtime fallback route generation/sourcing so canonical routes resolve from Sanity/query contracts only.
...
- [x] Connect `Sanity Generator V2` program preview and batch dry-run workflow in Studio plus smoke verification
- [x] Add `Sanity Generator V2` dev-only draft write path, write guard, and example development seed flow
- [x] Verify `Sanity Generator V2` frontend query/debug contract and live-dev smoke read path
...
- [x] Freeze legacy templating with exported generator migration inventory
- [x] Separate legacy templating from Generator V2 in the Studio desk and ignore recurring local temp artifacts
- [x] Trim unreachable legacy template branches and unused template payload fields without changing prod route allowlists
- [x] Minimize legacy template schemas in Studio by hiding inert fields and relabeling docs as legacy-only
- [x] Add development-only legacy-template-to-generator migration tooling and hide misleading `variant` editing
- [x] Replace the old dev generator seed with three aligned starter families and clean stale generator docs from the development dataset
- [x] Add CSV-ready dataset input flow and richer deterministic generator output for starter families
- [x] Rebase starter generator templates on the old generator pages as a visual section model
- [x] Replace service-specific development starters with a reusable multi-jasa visual template library in Sanity Studio
