...
- [x] Template onboarding is now less brand-locked: missing template resolver contract test restored, route allowlist made env-configurable, and global metadata/JSON-LD fallbacks now derive from env + Sanity settings instead of hardcoded `kotacom.id` constants.
- [x] Template lane model has been simplified for clean new projects: lane `generic` removed from Studio + frontend contracts, and `website` is now the primary/default universal lane (with legacy `generic` data normalized to `website` at runtime).
- [x] Template runtime path is now decoupled from legacy page shape on dynamic routes: template-backed pages use `RewritePageContext`, no longer depend on legacy sibling lookup in catch-all routes, and `RewritePageShell` now runs in template-first mode (neutral lane fallback copy + no legacy override fetch for template documents).
...
- [x] OG generate-save document action is now available across core public types (`post`, `page`, `service`, `product`, `project`) with type-aware OG badge labeling.
- [x] OG theme controls now support advanced customization in SEO Settings (font URL/family, typography scale, spacing, border/radius, overlay intensity) with safe renderer fallbacks.
- [x] OG configuration is now separated into dedicated `ogSettings` singleton (including live preview tab), while `seoSettings` remains focused on metadata/robots/sitemap concerns.
...
- [x] Ensure fallback global SEO data always available.
