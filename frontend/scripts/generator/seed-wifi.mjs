...
function template() {
  ...
  return {
    ...
    blocks: [
      ...
      { _type: "micro-badges-block", _key: `${id}-badges`, padding, colorVariant: "background", badges: [{ _key: `${id}-b1`, label: "Resmi 100%", description: "Pembayaran ke VA Provider" }, { _key: `${id}-b2`, label: "Proses Instan", description: "Cek coverage cuma 5 menit" }, { _key: `${id}-b3`, label: "Beragam Pilihan", description: "Indihome, FirstMedia, dll" }] },
      ...
      { _type: "metrics-rail-block", _key: `${id}-metrics`, padding, colorVariant: "primary", items: [{ _key: `${id}-m1`, value: cfg.metric, label: cfg.metricLabel, brand: "Pengalaman" }, { _key: `${id}-m2`, value: "4.8/5", label: "Rating kepuasan pelayanan", brand: "Kepercayaan" }, { _key: `${id}-m3`, value: "0 Rupiah", label: "Biaya administrasi calo", brand: "Transparan" }] },
      { _type: "process-faq-block", _key: `${id}-faq`, padding, colorVariant: "background", processTitle: "Pertanyaan Seputar Pemasangan", processSteps: [], faqTitle: "FAQ", faqs: cfg.faq.map((x, i) => ({ _key: `${id}-faq-${i}`, question: x[0], answer: [block(x[1], `${id}-faq-${i}-ans`)] })) },
